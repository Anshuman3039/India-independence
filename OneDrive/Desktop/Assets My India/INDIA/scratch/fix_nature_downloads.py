...
queries = {
    "eco-desert.jpg": "Thar Desert vegetation",
    "eco-wetlands.jpg": "Keoladeo National Park birds",
    "people-rivers.jpg": "Varanasi boats Ganges",
    "people-forests.jpg": "tendu leaves collection forest India",
    "eco-forest.jpg": "Kanha Sal forest trees",
    "wildlife-tiger.jpg": "Panthera tigris Ranthambore wild"
}
...
def search_commons(keyword):
    ...
    try:
        ...
        for res in results:
            title = res.get("title")
            if title.lower().endswith(('.jpg', '.jpeg', '.png')) and "pdf" not in title.lower() and "book" not in title.lower() and "zoo" not in title.lower() and "captive" not in title.lower():
                return title
    except Exception as e:
